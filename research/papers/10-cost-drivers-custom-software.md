...
Every external system yours must talk to - accounting, ERP, payment gateways, messaging, lab instruments, legacy databases - adds a seam, and seams cost three times: the connection, the error handling for when the other side misbehaves (it will), and the testing of both. Integrations also interact, and in our experience the second integration adds more than the first did, because the failure combinations to be handled and tested multiply rather than add. Treat that as practitioner judgment to test against your own vendor's estimate, not as a costing rule.
...
Two systems with identical screens can differ substantially in cost because one must satisfy an auditor, regulator, or accreditation body and the other must not. Defensible audit trails, strict access control, tamper-evidence, retention rules, high-availability targets - these shape the *architecture*, not just the feature list. That is the mechanism behind the retrofit warning: adding them later is not adding a feature but changing structural decisions that other code already depends on, which is why we advise pricing them from the start. How much more it costs depends entirely on how far the existing architecture is from what the requirement needs; we have seen it range from awkward to effectively a rebuild, and no honest single multiplier exists.
...
This is why hourly rate is a misleading comparison axis on its own. In the engagements we have seen - including systems we were called in to repair - a junior-heavy team quotes a lower rate and can deliver a higher total: more rework, more supervision overhead, more defects surfacing after handoff, and the part that appears in no quote, a system that is more expensive to change for the rest of its life. We state that as a pattern we have observed, not as a law: a well-supervised mixed team with strong review can do excellent work, and seniority poorly deployed wastes money at a higher rate. We lead and review with senior engineers because it is how we control rework, and we have an obvious commercial interest in the argument. So do not take our word for it: ask any vendor not for their rate but for how they control rework, and judge whether the answer is specific.

## Why quotes vary 5-10x for the "same" brief

Put the drivers together and the mystery of divergent quotes mostly dissolves. Across our own engagements and the quote sets clients have shown us, a spread of five to ten times between the highest and lowest quote for one brief is unremarkable. That figure is a ManiarTech observation from our own sample, not an industry measurement, and we have not seen a published dataset that would establish it as one. What we can defend is the mechanism, and it has three parts, none of which require anyone to be lying.
...
Be precise about what that model does and does not say. The cone describes how one estimate's accuracy improves as a single project's decisions get made; it is not a measurement of how far apart different vendors' quotes fall. What it does establish is the underlying condition: at brief stage the information needed to price the work does not yet exist, so wide divergence is the expected state rather than evidence of misconduct. Our observed 5-10x spread is consistent with that condition - it is not predicted by the cone, and we do not present it as such.
...
## Where cost hides: the work after the demo works
...
When a project "suddenly" overruns late, this is often what happened: the demonstrable part of the system was mistaken for most of the work. Nothing went wrong in delivery - the estimate simply priced the demo. When you review a quote, ask where each of these items lives. If they are nowhere, you have found the likely gap between the number you were quoted and the number you will pay.
...
- **Cutting non-functional depth in a regulated domain.** Retrofitting audit trails, access control, or security into an architecture not built for them means revisiting structural decisions rather than adding a feature - and the bill arrives when an auditor is waiting, which is the worst possible moment to be negotiating scope.
...
- **Cutting contingency.** The unknowns do not disappear when unpriced; they return as change orders - priced at a point where you have already committed, the vendor knows your switching cost, and you have the least leverage you will ever have.
- **Cutting seniority to cut rate.** As argued above: in our experience this raises the total while lowering the hourly number. It is our most consistent observation about staffing economics, and still an observation - ask any vendor to show you how they control rework rather than taking either side's word for it.
- **Thinning testing at the end.** The end is where the exception paths, failure modes, and migration verification live; testing is how they get found before your users find them.
