...
- **Content ordering is strictly deterministic.** Documentation trees follow numeric filename prefixes (stripped from the published URLs), and list pages sort by an explicitly declared key - because "mostly alphabetical" is tolerable on a five-page demo and maddening on a two-hundred-page site.
...
If that is the kind of engineering relationship you want, tell us what you are building - [get a free estimate](/estimate/), and a senior engineer reviews it and responds with a ballpark and an honest take.
