...
(function () {
    ...
    // The custom mouse-follower cursor is optional. Only wire it when the .cursor
    // element is present — otherwise editCursor() dereferences null on every
    // mousemove and throws (this build ships without a custom cursor).
    if (cursor) {
    ...
    elements.forEach((element) => {
        ...
        const rollClone = textContainer.cloneNode(true);
        element.appendChild(rollClone);

        // a11y: the letters are duplicated for the roll animation, so screen
        // readers announce each label twice ("HomeHome"). Expose one clean
        // accessible name and hide the decorative letter spans.
        element.setAttribute("aria-label", innerText);
        textContainer.setAttribute("aria-hidden", "true");
        rollClone.setAttribute("aria-hidden", "true");
    });
...
$(function () {
    ...
    /* Preloader timeline REMOVED. The preloader markup was dropped during the
       de-gimmick pass, but its GSAP intro still ran and hid the hero —
       `header .container` started at opacity:0 and only revealed after the
       ~3s loader sequence (those were the ".loader-wrap not found" warnings).
       With it gone, the hero renders immediately. */
...
$(function () {
    ...
    if (width > 991) {
        ...
        // Smooth-footer reveal — only when the theme's .footer-container exists.
        // Our footer is .mt-footer, so this is skipped (avoids a GSAP target warning).
